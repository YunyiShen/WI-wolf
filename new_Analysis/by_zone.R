...
focused_zone <- c("1","2","5")

for(zone in focused_zone){
  temp <- wi_zone[wi_zone$Zone==zone, ]
  
  logistic_fit <- nls(Wolves~p0*exp(r*(Year-1987))/(1+(p0*invK)*(exp(r*(Year-1987))-1)),
...
png("./figs/pop_vs_range_by_zone.png", width = 6, height = 3.5, res = 500, unit = "in")
...
legend("topleft",legend = c("zone 1","zone 2",
                            "zone 5",

                            "overall fit"), 
       lty = c(NA,NA,NA,1), pch = c(1,2,5,NA))
dev.off()
